...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const currentCategory =
    categoryArticles.length > 0 ? categoryArticles[0].category?.name : slug;
  ...
  return (
    <main className="container mx-auto py-10 px-4 bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="flex justify-between items-end mb-8 border-b pb-4 border-slate-300 dark:border-slate-700">
        <h1 className="text-4xl font-bold capitalize">{currentCategory}</h1>
      ...
      {categoryArticles.length > 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {categoryArticles.map((article, index) => (
            <div
              key={article.id}
              className="border rounded-lg overflow-hidden shadow-sm flex flex-col bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 transition-transform hover:scale-[1.02]"
            >
